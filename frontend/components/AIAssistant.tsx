...
import { Send, X, Bot, User, Brain, Search, FileText, AlertTriangle, CheckCircle, Zap, ToggleLeft, ToggleRight } from 'lucide-react';
...
const AIAssistant = React.forwardRef<AIAssistantRef, AIAssistantProps>(({ isOpen, onToggle, onFileUpload, onRepoLink }, ref) => {
  ...
  const checkForModeCommands = (message: string): boolean => {
    ...
    if (ragCommands.some(cmd => lowerMessage.includes(cmd))) {
      if (!isRAGMode) {
        setIsRAGMode(true);
        setMessages(prev => [...prev, {
          id: `mode-switch-${Date.now()}`,
          type: 'ai',
          content: '🔄 **Switched to RAG Mode!**\n\nNow using n8n webhook for responses. You can ask me anything and I\'ll process it through our intelligent RAG system.',
          timestamp: new Date()
        }]);
        return true;
      } else {
        setMessages(prev => [...prev, {
          id: `mode-already-${Date.now()}`,
          type: 'ai',
          content: '✅ **Already in RAG Mode!**\n\nI\'m already using the n8n webhook for responses. You can ask me anything!',
          timestamp: new Date()
        }]);
        return true;
      }
    ...
    if (aiCommands.some(cmd => lowerMessage.includes(cmd))) {
      if (isRAGMode) {
        setIsRAGMode(false);
        setMessages(prev => [...prev, {
          id: `mode-switch-${Date.now()}`,
          type: 'ai',
          content: '🔄 **Switched to AI Mode!**\n\nNow using GPT-4 for deployment analysis. I can help you with Docker, GitHub repositories, and deployment guidance.',
          timestamp: new Date()
        }]);
        return true;
      } else {
        setMessages(prev => [...prev, {
          id: `mode-already-${Date.now()}`,
          type: 'ai',
          content: '✅ **Already in AI Mode!**\n\nI\'m already using GPT-4 for deployment analysis. How can I help you?',
          timestamp: new Date()
        }]);
        return true;
      }
  ...
  // Update welcome message when RAG mode changes
  useEffect(() => {
    setMessages(prev => prev.map(msg => 
      msg.id === '1' 
        ? { 
            ...msg, 
            content: `🚀 Welcome to ZeroOps Assistant!\n\nI'm here to help you with intelligent deployment analysis. I can:\n\n• Analyze GitHub repositories in real-time\n• Generate production-ready Dockerfiles\n• Provide step-by-step deployment guidance\n• Answer technical questions\n\n💡 **Switch modes easily:**\n• **Use the RAG toggle** in the main navbar\n• **Or type commands** like:\n  - \"switch to rag mode\" or \"change to ai mode\"\n  - \"enable rag\" or \"use gpt\"\n\n🔧 **Current Mode**: ${isRAGMode ? 'RAG Mode (n8n)' : 'AI Mode (GPT-4)'}\n\nReady to get started? Just paste a GitHub URL or upload your project!`
          }
        : msg
    ));
  }, [isRAGMode]);
  ...
  const handleRAGQuery = async (userMessage: string) => {
    setIsRAGLoading(true);
    
    try {
      const response = await fetch('/api/rag-bot/query', {
        method: 'POST',
        headers: {
      ...
      setMessages(prev => [...prev, {
        ...
        type: 'ai',
        content: `❌ **RAG Error:** ${error instanceof Error ? error.message : 'Unknown error occurred'}`,
        timestamp: new Date()
      }]);
    } finally {
  ...
  const handleSendMessage = async () => {
    if (!inputValue.trim() || isThinking || isAnalyzing || isStreaming || isRAGLoading) return;
    ...
    if (checkForModeCommands(userMessage)) {
      return; // Don't process further if it was a mode command
    }

    // Add mode indicator to user message
    setMessages(prev => prev.map(msg => 
      msg.id === `user-${Date.now()}` 
        ? { ...msg, content: `${userMessage} ${isRAGMode ? '(RAG Mode)' : '(AI Mode)'}` }
        : msg
    ));

    if (isRAGMode) {
      await handleRAGQuery(userMessage);
    } else {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-full lg:w-96 bg-slate-900/95 backdrop-blur-sm border-r border-slate-700 transform transition-transform duration-300 ease-in-out z-40 ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">
                  {isRAGMode ? 'Powered by n8n RAG' : 'Powered by GPT-4'}
                </p>
              </div>
            </div>
          ...
          <div className="p-4 border-t border-slate-700 bg-slate-800/50">
            <div className="flex gap-2">
              <Input
                ...
                placeholder={isRAGMode ? "Ask RAG Bot anything..." : "Paste GitHub URL or ask a question..."}
                className="flex-1 bg-slate-700/50 border-slate-600 text-white placeholder:text-slate-400 focus:border-purple-400"
                disabled={isThinking || isAnalyzing || isStreaming || isRAGLoading}
              />
              <Button
                onClick={handleSendMessage}
                disabled={!inputValue.trim() || isThinking || isAnalyzing || isStreaming || isRAGLoading}
                className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-3"
              >
                <Send className="w-4 h-4" />
              </Button>
            </div>
            {(isThinking || isAnalyzing || isStreaming || isRAGLoading) && (
              <p className="text-xs text-slate-400 mt-2 text-center">
                {isRAGLoading ? '🤖 RAG Bot processing...' : isStreaming ? '📡 Streaming analysis...' : isAnalyzing ? '🔍 Analyzing project...' : '🧠 AI is thinking...'}
              </p>
            )}
          </div>
        </Card>
